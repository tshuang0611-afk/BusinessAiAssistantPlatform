...
CREATE TABLE wallets (
	...
	currency_type varchar(20) DEFAULT 'POINTS'::character varying NULL,
...
-- 1. 補齊缺失的 AI 診斷欄位 (確保名稱與程式碼一致)
ALTER TABLE assets_log ADD COLUMN IF NOT EXISTS ai_score int4;
ALTER TABLE assets_log ADD COLUMN IF NOT EXISTS ai_analysis text;
ALTER TABLE assets_log ADD COLUMN IF NOT EXISTS status varchar(20) DEFAULT 'PENDING';

-- 2. 強制轉換 ai_tags 為 jsonb (解決截圖中的類型衝突)
ALTER TABLE assets_log 
ALTER COLUMN ai_tags TYPE jsonb 
USING (to_jsonb(ai_tags));

ALTER TABLE assets_log ALTER COLUMN ai_tags TYPE jsonb USING to_jsonb(ai_tags);